...
-  A new module ``GHC.Stack.Types`` was added, exporting two new types
   ``SrcLoc`` and ``CallStack``. A ``SrcLoc`` contains package, module,
   and file names, as well as start and end positions.

   A new type ``CallStack`` was added for use with the new implicit callstack
   parameters. A ``CallStack`` is a ``[(String, SrcLoc)]``, sorted by
   most-recent call.
